...
<hr style='margin:1px 0px;'/>
  <form method='post'>
    <div class='row' style='margin-bottom:5px;padding-top:2px'>
      ...
      <div class='col-sm-3'>
        ...
        <?php
            ...
            $deptTimetendencyLink = $this->createLink('report', 'timetendency', "userRootId=$userRootId&amibaName=amibaName&isAmibaChanged=false&groupName=groupName&account=account&orgType=dept&timeType=日&endNum=$endNum&beginNum=");
      ...
      <div class='col-sm-3'>
        <div style='margin-top:7px;'>
        <?php echo $lang->report->reported . $lang->report->tasktime . '(' . $deptAllTime . '小时)';?>
        </div>
      </div>
    </div>
    ...
    <div class='row' style='margin-bottom:5px;'>
      ...
      <div class='col-sm-3'>
        <div class='input-group input-group-sm'>
          <span class='input-group-addon'><?php echo $lang->report->day;?></span>
  ...
  <table class='table table-condensed table-striped table-bordered tablesorter table-fixed active-disabled' id='dayreport'>
    <thead>
    <tr class='colhead'>
      <th width='160'><?php echo $lang->report->amiba_name;?></th>
      <th width='160'><?php echo $lang->report->group_name;?></th>
      <th width='160'><?php echo $lang->report->realname;?></th>
      <th width='160'><?php echo $lang->report->total_time . '（' . $lang->report->day . '）';?></th>
    </tr>
    </thead>
    <tbody>
    <?php $group_count = 0;?>
    <?php foreach($amibas as $amiba):?>
      <tr class="a-center">
        ...
        <td align='left' rowspan="<?php echo $amiba_user_count;?>">
            ...
            $endNum = date('Ymd',strtotime($day));
            $amibaTimetendencyLink = $this->createLink('report', 'timetendency', "userRootId=$userRootId&amibaName=$amiba->amiba_name&isAmibaChanged=false&groupName=groupName&account=account&orgType=amiba&timeType=日&endNum=$endNum&beginNum=");
        ...
        <?php foreach($amiba->groups as $group):?>
            ...
            <td align='left' style='<?php echo $group_count % 2 === 1 ?  'background-color:#f2f2f2' : 'background-color:white';?>' rowspan="<?php echo $group_user_count;?>">
                ...
                $endNum = date('Ymd',strtotime($day));
                $groupTimetendencyLink = $this->createLink('report', 'timetendency', "userRootId=$userRootId&amibaName=$amiba->amiba_name&isAmibaChanged=false&groupName=$group->group_name&account=account&orgType=group&timeType=日&endNum=$endNum&beginNum=");
                ...
                <?php foreach($group->users as $user):?>
                  <?php if($userindex != 1) echo "<tr class='a-center'>"?>
                    <td>
                        ...
                        $endNum = date('Ymd',strtotime($day));
                        $personTimetendencyLink = $this->createLink('report', 'timetendency', "userRootId=$userRootId&amibaName=amiba&isAmibaChanged=false&groupName=groupName&account=$user->account&orgType=person&timeType=日&endNum=$endNum&begin=");
                    ...
                    $worklogsLink = $this->createLink('report', 'worklogs', "account=$user->account&dateNum=$dateNum&timeType=日");
                    ...
                    $dailyworkLink = $this->createLink('report', 'dailywork', "account=$user->account");
