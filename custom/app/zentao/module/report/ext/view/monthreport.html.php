...
<div class='side'>
  ...
  <?php
      ...
      $monthReportExportLink = $this->createLink('report', 'monthreportexport', "userRootId=$userRootId&monthNum=$monthNum");
      print(html::a($monthReportExportLink, '导出', '', 'class="iframe btn btn-sm"'));
   ?>
   
  <?php if(common::hasPriv('report', 'recalperformance')):?>
...
<?php
$iconHtml = "<span class='tendency-icon' style='background-image:url(" . $defaultTheme . "images/ext/stat-feature-1077.png)'></span>";
?>
<div class='main' id='monthwork'>
 <hr style='margin:1px 0px;'/>
    ...
    <div class='row' style='margin-bottom:5px;padding-top:2px'>
      ...
      <div class='col-sm-2'>
        ...
        <?php
            ...
            $deptTimetendencyLink = $this->createLink('report', 'timetendency', "userRootId=$userRootId&amibaName=amibaName&isAmibaChanged=false&groupName=groupName&account=account&orgType=dept&timeType=月&endNum=$monthNum&beginNum=");
      ...
      <div class='col-sm-2'>
        <div style='margin-top:7px;' data-container='body' data-toggle='popover' data-placement='top' data-trigger='hover' data-content='<?php 
            echo $lang->report->standard . $lang->report->tasktime. ' = ' . '人数 * 天数 * 8';
            ?>'>
            <?php 
          $deptStandardTime = $deptUserCount * $workDayCount * 8;
          echo $lang->report->standard . $lang->report->tasktime . '(' . $deptStandardTime . ')';
          ?>
        </div>
      </div>
      <div class='col-sm-2'>
        <div style='margin-top:7px;' >
            <?php echo $lang->report->reported . $lang->report->tasktime . '(' . $deptAllTime . ')';?>
        </div>
      </div>
      <div class='col-sm-2'>
        <div style='margin-top:7px;' >
            <?php 
                ...
                echo $lang->report->day_avg . $lang->report->tasktime . '(' . $dayavgtime . ')';
              ?>
        </div>
      </div>
      <div class='col-sm-2'>
        <div style='margin-top:7px;'  data-container='body' data-toggle='popover' data-placement='top' data-trigger='hover' data-content='<?php 
            ...
            <?php 
                ...
                echo $lang->report->extra . $lang->report->tasktime . '(' . $extrtime . ')';
              ?>
        </div>
      </div>
    </div>
  ...
  <table class='table table-condensed table-striped table-bordered tablesorter active-disabled' id='monthreport'>
    <thead>
        <tr>
          <th width='220' colspan='4'><?php echo $lang->report->amiba_name;?></th>
          <th width='210' colspan='4'><?php echo $lang->report->group;?></th>
          <th width='460' colspan='8'><?php echo $lang->report->person;?></th>
        </tr>
        
        <tr>
          <th width='70'><?php echo $lang->report->name;?></th>
          <th width='50'><?php echo $lang->report->reported . $lang->report->tasktime;?></th>
          <th width='50'><?php echo $lang->report->output . $lang->report->efficiency;?></th>
          <th width='50'><?php echo $lang->report->person_avg . $lang->report->output;?></th>
          
          
          <th width='60'><?php echo $lang->report->name;?></th>
          <th width='50'><?php echo $lang->report->reported . $lang->report->tasktime;?></th>
          <th width='50'><?php echo $lang->report->output . $lang->report->efficiency;?></th>
          <th width='50'><?php echo $lang->report->person_avg . $lang->report->output;?></th>
          ...
          <th width='60'><?php echo $lang->report->name;?></th>
          <th width='50'><?php echo $lang->report->reported . $lang->report->tasktime;?></th>
          <th width='50'><?php echo $lang->report->day_avg . $lang->report->tasktime;?></th>
          <th width='50' data-container='body' data-toggle='popover' data-placement='top' data-trigger='hover' data-content='<?php 
            echo $lang->report->extra . $lang->report->tasktime. ' = ' . $lang->report->reported . $lang->report->tasktime . ' - 标准工时';
            ?>'>
          <?php echo $lang->report->extra . $lang->report->tasktime;?>
          </th>
          ...
          <th width='50' style='color:red;'><?php echo $lang->report->real . $lang->report->output;?></th>
          <th width='50'><?php echo $lang->report->day_avg . $lang->report->output;?></th>
    ...
    <?php foreach($amibas as $amiba):?>
      <tr class="a-center">
        ...
        <?php 
            // 阿米巴-总工时
            $timetendencyLink = $this->createLink('report', 'timetendency', "userRootId=$userRootId&amibaName=$amiba->amiba_name&isAmibaChanged=false&groupName=groupName&account=account&orgType=amiba&timeType=月&endNum=$monthNum&beginNum=");
        ...
        if(!empty($amiba->total_output) && !empty($amiba->total_time))
        {
            ...
            echo $lang->report->output . $lang->report->efficiency . '(' . $amibaEfficiency . ')' . '=' . $lang->report->total . $lang->report->output . '(' . $amiba->total_output . ')/' . $lang->report->total . $lang->report->tasktime . '(' . $amiba->total_time . ')' ;
        }
        ?>'>
        <?php
            ...
            if(!empty($amiba->total_output) && !empty($amiba->total_time))
            {
                $monthPerformanceLink = $this->createLink('report', 'monthperformance', 
        ...
        <?php foreach($amiba->groups as $group):?>
            ...
            <?php 
                // 组-总工时
                $timetendencyLink = $this->createLink('report', 'timetendency', "userRootId=$userRootId&amibaName=$amiba->amiba_name&isAmibaChanged=false&groupName=$group->group_name&account=account&orgType=group&timeType=月&endNum=$monthNum&beginNum=");
            ...
            if(!empty($group->total_output) && !empty($group->total_time))
            {
                ...
                echo $lang->report->output . $lang->report->efficiency . '(' . $amibaEfficiency . ')' . '=' . $lang->report->total . $lang->report->output . '(' . $group->total_output . ')/' . $lang->report->total . $lang->report->tasktime . '(' . $group->total_time . ')' ;
            }
            ?>'>
            <?php 
            ...
            <?php foreach($group->users as $user):?>
              <?php if($userindex != 1) echo "<tr class='a-center'>"?>
                ...
                <?php 
                    ...
                    {
                        $timetendencyLink = $this->createLink('report', 'timetendency', "userRootId=$userRootId&amibaName=amiba&isAmibaChanged=false&groupName=groupName&account=$user->account&orgType=person&timeType=月&endNum=$monthNum&beginNum=");
                ...
                {
                    echo $lang->report->output . $lang->report->efficiency . '(' . $user->output_efficiency . ')' . '=' . $lang->report->total . $lang->report->output . '(' . $user->total_output . ')/' . $lang->report->total . $lang->report->tasktime . '(' . $user->total_time . ')' ;
                }
                ?>'>
                <?php
                    // 个人-输出效率
...
function recalPerformanceTip(){
    var msg = 
        "<?php
            // 获取服务地址
            $performanceServiceUrl = $config->report->performanceServiceUrls[$config->worklog->depcode];
