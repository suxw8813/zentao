    ...
    <div class='main'>
      <div class='tabs'>
        <ul class='nav nav-tabs'>
		  ...
		  <li class='<?php if($type == 'task') echo 'active'?>'><a href='#tasks' data-toggle='tab'><?php echo  html::icon($lang->icons['task']) . ' ' . $lang->productplan->linkedTasks;?></a></li>
		  <li class='<?php if($type == 'testcase') echo 'active'?>'><a href='#testcases' data-toggle='tab'><?php echo  html::icon($lang->icons['testcase']) . ' ' . $lang->productplan->linkedTestcases;?></a></li>
          <?php endif;?>
        ...
        <div class='tab-content'>
          ...
          <div id='tasks' class='tab-pane <?php if($type == 'task') echo 'active';?>'>
            <form method='post' target='hiddenwin'">
              <table class='table tablesorter table-condensed table-hover table-striped table-fixed table-selectable' id='taskList'>
                <?php $vars = "planID={$plan->id}&type=task&orderBy=%s&link=$link&param=$param"; ?>
                <thead>
                <tr>
                  ...
                  <th class='w-status {sorter:false}'>  <?php common::printOrderLink('status',   $orderBy, $vars, $lang->statusAB);?></th>
                  <th class='w-user {sorter:false}'>  <?php common::printOrderLink('deadline', $orderBy, $vars, $lang->task->deadlineAB);?></th>
				  <th class='{sorter:false}'>         <?php common::printOrderLink('title',      $orderBy, $vars, $lang->productplan->storyTitle);?></th>
                </tr>
                </thead>
                <tbody>
                  <?php foreach($planTasks as $task):?>
                  <tr class='text-center'>
				    
                    <td class='cell-id'>
                      <?php echo html::a($this->createLink('task', 'view', "taskID=$task->id"), sprintf("%03d", $task->id));?>
                    </td>
                    <td><span class='<?php echo 'pri' . zget($lang->task->priList, $task->pri, $task->pri)?>'><?php echo $lang->task->priList[$task->pri];?></span></td>
					...
					<td class='text-left nobr' title='<?php echo $task->title?>'><?php echo $task->title;?></td>
                  </tr>
                  <?php endforeach;?>
                </tbody>
                <tfoot>
                <tr>
                  <td colspan='6'>
                    <div class='table-actions clearfix'>
                      <div class='text'><?php echo sprintf($lang->productplan->taskSummary, count($planTasks));?> </div>
                    </div>
                  </td>
                </tr>
                </tfoot>
          ...
          <div id='testcases' class='tab-pane <?php if($type == 'testcase') echo 'active';?>'>
            <form method='post' target='hiddenwin'">
              <table class='table tablesorter table-condensed table-hover table-striped table-fixed table-selectable' id='caseList'>
                <?php $vars = "planID={$plan->id}&type=testcase&orderBy=%s&link=$link&param=$param"; ?>
                <thead>
                <tr>				  
                  <th class='w-id {sorter:false}'>    <?php common::printOrderLink('id',         $orderBy, $vars, $lang->idAB);?></th>
                  <th class='w-pri {sorter:false}'>   <?php common::printOrderLink('pri',        $orderBy, $vars, $lang->priAB);?></th>				  
                  <th class='{sorter:false}'>         <?php common::printOrderLink('title',      $orderBy, $vars, $lang->productplan->testcaseTitle);?></th>				  
				  ...
				  <th class='{sorter:false}'>         <?php common::printOrderLink('story',      $orderBy, $vars, $lang->productplan->storyTitle);?></th>
                </tr>
                </thead>
                <tbody>
                  <?php foreach($planCases as $case):?>
                  <tr class='text-center'>
				    
                    <td class='cell-id'><?php echo html::a($this->createLink('testcase', 'view', "caseID=$case->id"), sprintf("%03d", $case->id));?></td>
                    <td><span class='<?php echo 'pri' . zget($lang->testcase->priList, $case->pri, $case->pri)?>'><?php echo zget($lang->testcase->priList, $case->pri, $case->pri);?></span></td>					
                    <td class='text-left nobr' title='<?php echo $case->title?>'><?php echo html::a($this->createLink('testcase', 'view', "caseID=$case->id"), $case->title);?></td>
					<td><?php echo $lang->testcase->typeList[$case->type];?></td>
					<td><?php echo $users[$case->openedBy];?></td>
					<td><?php echo $lang->testcase->statusList[$case->status];?></td>
					<td class='text-left nobr' title='<?php echo $case->storyTitle?>'><?php echo $case->storyTitle;?></td>
                  </tr>
                  <?php endforeach;?>
                </tbody>
                <tfoot>
                <tr>
                  <td colspan='6'>
                    <div class='table-actions clearfix'>
                      <div class='text'><?php echo sprintf($lang->productplan->testcaseSummary, count($planCases));?> </div>
                    </div>
                  </td>
                </tr>
                </tfoot>
